...
let running = []
let color = ["#fa5a5a", "#f0d264", "#82c8a0", "#7fccde", "#6698cb", "#cb99c5"]
...
class Line extends React.Component {
    ...
    render() {
        ...
        lists = lists.sort((a, b) => {
            let mou = a.conEnd.toArray()[1]
            let mou2 = b.conEnd.toArray()[1]
            let day = a.conEnd.toArray()[2]
            let day2 = b.conEnd.toArray()[2]
            if (mou !== mou2)
                return mou2 - mou
            else {
                return day2 - day
        ...
        for (let i = 0; i < lists.length; i++){
            ...
            if (mouBe === mouth - 1 && daBe === day) {
                ...
                u.push((<span style={{
                    ...
                    <div style={{display: "inline-block",height: 20, backgroundColor: col}}>
                        <span style={{
                            overflow: "hidden",
                            width: "105px",height: 20
                            , backgroundColor: col, marginLeft: "5px", display: "inline-block",
                        }}> {lists[i].name}
                    </span></div>

                   </span>))
            } else if (mouBe <= mouth - 1 && mouEn >= mouth - 1) {
                const cols=lists[i].color
                if (mouBe === mouth - 1 && mouEn === mouth - 1) {
                    if (daBe <= day && daEn >= day) {
                        u.push(<span style={{
                            display: "inline-block", marginBottom: 2, position: "relative"
                            , clear: "both",
                            float: "left", backgroundColor: cols, width: "128px", height: 20
                        }}/>)
                    } else {
                        u.push(<span></span>)
                    }
                    ...
                    if (daEn >= day) {
                        u.push(<span style={{
                            display: "inline-block", marginBottom: 2, position: "relative", clear: "both",
                            float: "left", backgroundColor: cols, width: "128px", height: 20
                        }}/>)
                    } else {
                        u.push(<span></span>)
        ...
        return (<div style={{position: "absolute", clear: "both", float: "left"}}> {u.map((value, index) => {
            return <div key={index} style={{clear: "both", float: "left"}}>{value}</div>
        })}</div>)
    }
}
